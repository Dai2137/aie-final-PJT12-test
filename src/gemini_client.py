...
class GeminiClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        Args:
            api_key: Gemini API キー（環境変数から取得される場合はNone）
        """
        ...
        logger.info("Gemini API クライアントを初期化しました")
    ...
    def analyze_profile(self, profile_text: str, requirements: Dict) -> Dict:
        ...
        try:
            ...
            logger.info(f"プロフィール分析完了: マッチ度 {result.get('match_score', 0)}")
            
            return result
            
        except Exception as e:
            logger.error(f"プロフィール分析エラー: {e}")
            return {
                "match_score": 0,
                "potential_score": 0,
